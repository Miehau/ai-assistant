...
2. Launch **2–3 code-explorer agents** in parallel (use `subagent_type: "harness-code-explorer"`). Each explorer should investigate a different aspect of the feature surface (e.g., frontend entry point, backend handler, data model).
...
1. Launch **2–3 code-architect agents** (use `subagent_type: "harness-code-architect"`). Each architect should explore a different design approach or focus on a different layer (frontend, backend, data).
...
1. Launch the **invariant-enforcer agent** (use `subagent_type: "invariant-enforcer"`).
...
1. Launch **3 code-reviewer agents** (use `subagent_type: "harness-code-reviewer"`). Each reviewer should focus on a different concern:
...
1. Launch the **entropy-scanner agent** (use `subagent_type: "entropy-scanner"`).
...
2. Launch the **knowledge-gardener agent** (use `subagent_type: "knowledge-gardener"`).
...
1. Launch the **invariant-enforcer agent** again (use `subagent_type: "invariant-enforcer"`).
