<script lang="ts">
  ...
  function exportData(format: 'json' | 'csv') {
    ...
    if (format === 'json') {
      ...
      downloadBlob(new Blob([rows.join('\n')], { type: 'text/csv' }), 'usage-statistics.csv');
    }
  }
  ...
  async function backfillUsage() {
    ...
    if (!confirm('Backfill missing usage records for existing conversations? This may take a moment.')) return;
    backfilling = true;
    backfillError = null;
    backfillResult = null;
    try {
  ...
  const costChartData = $derived(
    statistics?.by_date
      .map(d => ({ date: new Date(d.date), cost: d.total_cost }))
      .sort((a, b) => a.date.getTime() - b.date.getTime()) ?? []
  );

  const tokenChartData = $derived(
    statistics?.by_date
      .map(d => ({ date: new Date(d.date), tokens: d.total_tokens }))
      .sort((a, b) => a.date.getTime() - b.date.getTime()) ?? []
  );
...
<div class="p-6 max-w-4xl mx-auto">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-6 surface-card-quiet px-3 py-2">
    <Button variant={dateRange === '7d' ? 'default' : 'outline'} size="sm" onclick={() => dateRange = '7d'} class={dateRange === '7d' ? 'bg-white/10 border-white/15' : ''}>Last 7 Days</Button>
    <Button variant={dateRange === '30d' ? 'default' : 'outline'} size="sm" onclick={() => dateRange = '30d'} class={dateRange === '30d' ? 'bg-white/10 border-white/15' : ''}>Last 30 Days</Button>
    <Button variant={dateRange === 'all' ? 'default' : 'outline'} size="sm" onclick={() => dateRange = 'all'} class={dateRange === 'all' ? 'bg-white/10 border-white/15' : ''}>All Time</Button>
    <div class="ml-auto flex gap-2">
      <Button variant="outline" size="sm" class="border-white/10 hover:bg-white/5" onclick={backfillUsage} disabled={backfilling}>
  ...
  {:else if statistics}
    ...
    {:else if backfillResult}
      <div class="surface-card p-4 mb-6">
        <p class="text-sm text-muted-foreground/80">
          Backfill complete: {backfillResult.messages_backfilled} messages updated across {backfillResult.conversations_updated} conversations.
        </p>
      </div>
    {/if}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      <div class="surface-card p-5">
        ...
        <p class="text-xs text-muted-foreground/70 mt-2">{Math.round(statistics.total_tokens / Math.max(statistics.total_messages, 1))} avg/message</p>
      </div>
      <div class="surface-card p-5">
        <div class="flex items-center justify-between mb-2">
        ...
        <p class="text-2xl font-semibold">${(statistics.total_cost / Math.max(statistics.total_messages, 1)).toFixed(4)}</p>
            ...
            <div class="surface-card-quiet p-3">
              <div class="flex justify-between items-center mb-2">
                <div>
                  ...
                  <p class="text-xs text-muted-foreground/70">{model.message_count} messages · {model.total_tokens.toLocaleString()} tokens</p>
                </div>
              ...
              <div class="h-1.5 bg-white/5 rounded-full overflow-hidden mb-1">
                <div class="h-full bg-white/20 transition-all duration-300" style="width: {costPercentage}%"></div>
              </div>
              <div class="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div class="h-full bg-white/10 transition-all duration-300" style="width: {tokenPercentage}%"></div>
              </div>
            </div>
          {/each}
        </div>
    ...
    {#if statistics.by_date.length > 0}
      <details class="surface-card p-6">
        <summary class="text-lg font-semibold mb-4 cursor-pointer hover:text-foreground/90 transition-colors">Daily Usage Details</summary>
            ...
            <div>
              <div class="flex justify-between items-center mb-1">
                ...
                <div class="text-right">
                  ...
                  <p class="text-xs text-muted-foreground">{daily.message_count} msgs · {daily.total_tokens.toLocaleString()} tokens</p>
                </div>
              </div>
              <div class="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div class="h-full bg-white/20 transition-all duration-300" style="width: {costPercentage}%"></div>
